...
public class OzoneCallQueueManager<E extends Schedulable>
  ...
  int getPriorityLevel(UserGroupInformation user) {
    if (scheduler instanceof OzoneDecayRpcScheduler) {
      return ((OzoneDecayRpcScheduler)scheduler).getPriorityLevel(user);
    }
    return 0;
  }
  ...
  void setPriorityLevel(UserGroupInformation user, int priority) {
    if (scheduler instanceof OzoneDecayRpcScheduler) {
      ((OzoneDecayRpcScheduler)scheduler).setPriorityLevel(user, priority);
    }
  }
  ...
  boolean addInternal(E e, boolean checkBackoff) {
    ...
    } catch (OzoneCallQueueManager.CallQueueOverflowException ex) {
  ...
  private void throwBackoff() throws IllegalStateException {
    throw serverFailOverEnabled ?
        OzoneCallQueueManager.CallQueueOverflowException.FAILOVER :
        OzoneCallQueueManager.CallQueueOverflowException.DISCONNECT;
  }
