...
/**
 * Tests for S3IdentityProvider.
 */
public class TestS3IdentityProvider {

  private static S3IdentityProvider s3IdentityProvider;
  private static Schedulable schedulable;

  @BeforeAll
  public static void setUp() {
    s3IdentityProvider = new S3IdentityProvider();
  ...
  public void testGetUserFromS3Auth() {
    ...
    when(s3Authentication.getAccessId())
        .thenReturn("s3ClientUser");

    OzoneManagerProtocolServerSideTranslatorPB.setS3Auth(s3Authentication);

    String identity = s3IdentityProvider.makeIdentity(schedulable);
  ...
  public void testGetUserFromUgi() {
    ...
    when(schedulable.getUserGroupInformation()
        .getShortUserName()).thenReturn("ugiUser");
