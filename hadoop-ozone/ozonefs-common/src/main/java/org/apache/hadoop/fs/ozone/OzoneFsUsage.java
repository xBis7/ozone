...
public class OzoneFsUsage extends FsCommand {
  public static void registerCommands(CommandFactory factory) {
    factory.addClass(OzoneFsUsage.Df.class, "-df");
    factory.addClass(OzoneFsUsage.Du.class, "-du");
    factory.addClass(OzoneFsUsage.Dus.class, "-dus");
  }

  private boolean humanReadable = false;
  private OzoneFsUsage.TableBuilder usagesTable;
  ...
  public OzoneFsUsage.TableBuilder getUsagesTable() {
    return usagesTable;
  }

  public void setUsagesTable(OzoneFsUsage.TableBuilder usagesTable) {
  ...
  public static class Df extends OzoneFsUsage {
    ...
    protected void processArguments(LinkedList<PathData> args)
        throws IOException {
      setUsagesTable(new OzoneFsUsage.TableBuilder(
  ...
  /** show disk usage */
  public static class Du extends OzoneFsUsage {
    ...
    protected void processArguments(LinkedList<PathData> args)
        throws IOException {
      if (showHeaderLine) {
        setUsagesTable(new OzoneFsUsage.TableBuilder("DISK_SPACE",
            "DISK_SPACE_WITH_ALL_REPLICAS", "FULL_PATH_NAME"));
      } else {
        setUsagesTable(new OzoneFsUsage.TableBuilder(3));
      }
      super.processArguments(args);
      if (!getUsagesTable().isEmpty()) {
        getUsagesTable().printToStream(out);
    ...
    protected void processPath(PathData item) throws IOException {
      ...
      long spaceConsumed = getReplicaSpaceConsumed(item);
      if (excludeSnapshots) {
    ...
    private long getReplicaSpaceConsumed(PathData item)
        throws IOException {
      Path path = item.path;
      String FILE_SEPARATOR = File.separator;
      String[] entities = path.toString().split(FILE_SEPARATOR);
      String volName = entities[0];
      ContentSummary contentSummary = item.fs.getContentSummary(item.path);
      String bucketName;

      if (entities.length > 1) {
        bucketName = entities[1];
      } else {
        bucketName = "";
      }
...
//      rpcClient.getKeysEveryReplicas(volName, bucketName, );
    ...
    private ConfigurationSource getConfSource() {
  ...
  public static class Dus extends OzoneFsUsage.Du {
