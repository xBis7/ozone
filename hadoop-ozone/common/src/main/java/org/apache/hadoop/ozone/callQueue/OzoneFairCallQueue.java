...
/**
 * Ozone implementation of Hadoop FairCallQueue.
 */
public class OzoneFairCallQueue<E extends Schedulable>
  ...
  public OzoneFairCallQueue(int priorityLevels, int capacity, String ns,
    ...
    LOG.info("OzoneFairCallQueue is in use with " + numQueues +
    ...
    OzoneFairCallQueue.MetricsProxy mp =
        OzoneFairCallQueue.MetricsProxy.getInstance(ns);
    mp.setDelegate(this);
  }
  ...
  public boolean add(E e) {
    ...
    if (!offerQueues(priorityLevel, e, true)) {
      ...
      OzoneFairCallQueue.CallQueueOverflowException ex;
      if (serverFailOverEnabled) {
        // Signal clients to failover and try a separate server.
        ex = OzoneFairCallQueue.CallQueueOverflowException.FAILOVER;
      } else if (priorityLevel == queues.size() - 1) {
        // only disconnect the lowest priority users that overflow the queue.
        ex = OzoneFairCallQueue.CallQueueOverflowException.DISCONNECT;
      } else {
        ex = OzoneFairCallQueue.CallQueueOverflowException.KEEPALIVE;
      }
      throw ex;
    }
    return true;
  ...
  private static final class MetricsProxy implements FairCallQueueMXBean,
    ...
    private static final HashMap<String, OzoneFairCallQueue.MetricsProxy>
        INSTANCES = new HashMap<>();

    // Weakref for delegate, so we don't retain it forever if it can be GC'd
    private WeakReference<OzoneFairCallQueue<? extends Schedulable>> delegate;
    ...
    public static synchronized OzoneFairCallQueue
        .MetricsProxy getInstance(String namespace) {
      OzoneFairCallQueue.MetricsProxy mp = INSTANCES.get(namespace);
      if (mp == null) {
        // We must create one
        mp = new OzoneFairCallQueue.MetricsProxy(namespace);
        INSTANCES.put(namespace, mp);
      }
      return mp;
    }

    public void setDelegate(OzoneFairCallQueue<? extends Schedulable> obj) {
      this.delegate
    ...
    private OzoneFairCallQueue<? extends Schedulable> getCallQueue() {
      WeakReference<OzoneFairCallQueue<? extends Schedulable>> ref =
          this.delegate;
      if (ref == null) {
        return null;
      }
    ...
    public int[] getQueueSizes() {
      OzoneFairCallQueue<? extends Schedulable> obj = getCallQueue();
      if (obj == null) {
        return new int[]{};
      }
    ...
    public void getMetrics(MetricsCollector collector, boolean all) {
      MetricsRecordBuilder rb = collector.addRecord("OzoneFairCallQueue")
          .setContext("rpc")
      ...
      for (int i = 0; i < currentQueueSizes.length; i++) {
        rb.addGauge(Interns.info("OzoneFairCallQueueSize_p" + i,
                "FCQ Queue Size"), currentQueueSizes[i]);
        rb.addCounter(Interns.info("OzoneFairCallQueueOverflowedCalls_p" + i,
            "FCQ Overflowed Calls"), currentOverflowedCalls[i]);
      }
    }
  }

  // OzoneFairCallQueueMXBean
  public int[] getQueueSizes() {
