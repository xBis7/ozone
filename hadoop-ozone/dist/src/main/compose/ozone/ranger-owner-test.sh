...
# Set native ACLs
echo ""
echo "hadoop | volume setacl /tmp | exp_res: 'ACLs set successfully.'"
docker-compose exec -T om ozone sh volume setacl -a user:testuser2:rw,user:testuser:a,group:testuser2:rw,group:testuser:a tmp

echo ""
echo "hadoop | bucket setacl /tmp/tmp | exp_res: 'ACLs set successfully.'"
docker-compose exec -T om ozone sh bucket setacl -a user:testuser2:rwlc,user:testuser:a,group:testuser2:rwlc,group:testuser:a tmp/tmp
...
then
  ...
  echo ""
  echo "testuser2 | fs -rm -skipTrash ofs://om/tmp/rootkey1 | exp_res: 'Input/output error'"
  docker-compose exec -T -u testuser2 om ozone fs -rm -skipTrash ofs://om/tmp/rootkey1 || true

  # 'testuser2' can delete his own file
  echo ""
  echo "testuser2 | fs -rm -skipTrash ofs://om/tmp/rootkey2 | exp_res: 'Deleted'"
  docker-compose exec -T -u testuser2 om ozone fs -rm -skipTrash ofs://om/tmp/rootkey2
  ...
  echo ""
  echo "testuser | fs -rm -skipTrash ofs://om/tmp/rootkey1 | exp_res: 'Deleted'"
  docker-compose exec -T -u testuser om ozone fs -rm -skipTrash ofs://om/tmp/rootkey1
